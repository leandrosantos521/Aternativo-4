...
async function handleInfinitePayWebhook(event){
  ...
  let uber=null;
  if(order.delivery?.method==='uber'){
    try{ uber=await createUberForPaidOrder(order.id); }catch(e){ console.error('[Uber após pagamento]',e); }
  }
  ...
  return json(200,{ok:true,status:'PAID',blingUpdated,order:updated,uber});
}
...
async function checkInfinitePayReturn(event){
  ...
  if(payment?.success===true && payment?.paid===true){
    ...
    if(paidAmount>=expected && order.status!=='PAID'){
      ...
      if(order.delivery?.method==='uber'){
        try{ await createUberForPaidOrder(id); }catch(e){ console.error('[Uber retorno InfinitePay]',e); }
      }
    }
  }
...
async function createUberForPaidOrder(id){
  const order=await resolveOrder(id);
  if(!order) throw new Error('Pedido não encontrado.');
  if(order.payment_status!=='APPROVED' && order.status!=='PAID') throw new Error('O pagamento ainda não foi aprovado.');
  if(!['uber','pickup_uber'].includes(order.delivery?.method)) return null;
  if(order.raw?.uber?.delivery_id) return order.raw.uber;
  if(order.delivery?.method==='pickup_uber' && String(order.raw?.fulfillmentStatus||'')!=='Liberado para Uber'){
    return null;
  }
  ...
  if(!quoteId) throw new Error('Não foi possível obter uma cotação do Uber Direct.');
...
async function requestUber(event){
  ...
  if(!['uber','pickup_uber'].includes(order.delivery?.method)) return json(409,{message:'Este pedido não usa Uber Direct.'});
  if(order.delivery?.method==='pickup_uber' && String(order.raw?.fulfillmentStatus||'')!=='Liberado para Uber') return json(409,{message:'A loja ainda não liberou o pedido para o Uber.'});
  try{
    const uber=await createUberForPaidOrder(id);
    return json(200,{ok:true,uber});
  }catch(e){ return json(409,{message:e.message||'Não foi possível solicitar o Uber Direct.'}); }
}
