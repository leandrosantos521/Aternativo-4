...
const {getOAuth,saveOAuth}=require('./_lib/melhor-envio-oauth-store');
...
function callback(){return String(process.env.MELHOR_ENVIO_CALLBACK_URL||'https://relppscos.netlify.app/melhor-envio-callback.html').trim()}
...
async function listCorreiosServiceIds(token){
  ...
  const wanted=list.filter(s=>{
    ...
    return /correios/i.test(company) && /\b(?:pac|sedex|mini\s*envios)\b/i.test(name.trim());
  });
