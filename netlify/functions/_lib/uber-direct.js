...
async function createDelivery({order,quoteId}){
  ...
  const payload={quote_id:quoteId,pickup_address:pickupAddress(),pickup_name:'Relpps Cosméticos',pickup_phone_number:pickupPhone(),dropoff_address:parseAddress(d),dropoff_name:String(c.name||'Cliente'),dropoff_phone_number:`+${String(c.phone||'').replace(/\D/g,'')}`,manifest_items:manifest,external_id:String(order.id),idempotency_key:`relpps-${String(order.id).replace(/[^a-zA-Z0-9_-]/g,'-')}`};
