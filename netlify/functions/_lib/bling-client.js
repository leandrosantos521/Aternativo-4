...
async function findOrCreateContactIdentity(customer){
  const rawCpf=String(customer?.cpf||'').trim();
  const cpf=cleanDoc(rawCpf);
  const cpfOk=validCPF(cpf);
  const email=String(customer?.email||'').trim().toLowerCase();
  if(email && !validEmail(email)) throw new Error('E-mail inválido.');
  let contact=null;
  if(cpfOk){
  ...
  const payload={
    ...
    ...(email?{email}:{}),
    ...(String(customer?.phone||'').replace(/\D/g,'')?{telefone:String(customer.phone).replace(/\D/g,'')}:{}),
  };
