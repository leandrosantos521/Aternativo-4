...
const { findOrCreateContactIdentity } = require("./_lib/bling-client");
...
async function verifySupabaseUser(event){
  ...
  if(!auth.startsWith('Bearer ')||!base||!key)return null;
  try{
...
exports.handler = async (event) => {
  try {
    ...
    if(action==="sync-contact" && method==="POST"){
      const authUser=await verifySupabaseUser(event);
      if(!authUser?.id) return json(401,{message:"Sessão do cliente não autorizada."});
      const payload=JSON.parse(event.body||"{}");
      const customer={...(payload.customer||{}),userId:authUser.id,email:authUser.email||payload.customer?.email};
      const contact=await findOrCreateContactIdentity(customer);
      if(!contact?.id) return json(502,{message:"O Bling não retornou o contato."});
      return json(200,{ok:true,contact:{id:contact.id,nome:contact.nome||customer.name,email:contact.email||customer.email,numeroDocumento:contact.numeroDocumento||customer.cpf||""}});
    }
